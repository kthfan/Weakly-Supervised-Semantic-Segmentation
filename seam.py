...
class SEAM(tf.keras.models.Model):
    ...
    # Usage:
        ...
        seam = SEAM(img_input, x, 10, correlation_feature=corr, min_pooling_rate=0.7)
    ...
    def _cnn_head(self, feature_output, correlation_feature, classes):
        if correlation_feature is None:
            correlation_feature = tf.keras.layers.Conv2D(192, 1, use_bias=False)(feature_output)
        ...
        cam_feature = self._non_max_suppress(cam_prob[:,:,:,:-1])
